...
/* FIXME: It may be better to put the `goban' argument into
 *	  `influence_data' structure and assert everywhere that we are
 *	  called for the position the influence is computed for (using
 *	  board hashes.)  /pp
 */
...
static void segment_influence(const Goban *goban, struct influence_data *q);
static void print_influence(const Goban *goban, const struct influence_data *q,
			    const char *info_string);
static void print_numeric_influence(const Goban *goban,
				    const struct influence_data *q,
				    const float values[BOARDMAX],
				    const char *format, int width,
				    int draw_stones, int mark_epsilon);
static void print_influence_areas(const Goban *goban,
				  const struct influence_data *q);

static void value_territory(const Goban *goban, struct influence_data *q);
static void enter_intrusion_source(const Goban *goban,
				   int source_pos, int strength_pos,
				   float strength, float attenuation,
				   struct influence_data *q);
static void add_marked_intrusions(const Goban *goban, struct influence_data *q,
				  int color);


/* THREAD-FIXME: Wrap these up somehow. */
...
#define code1(arg_di, arg_dj, arg, arg_d)\
  do {									\
    if (!q->safe[arg]							\
...
static void
accumulate_influence(const Goban *goban, struct influence_data *q,
		     int pos, int color)
{
  int ii;
  int m = I(pos);
  ...
  if (0) {
    gprintf(goban, "Accumulating influence for %s at %m\n",
  ...
  while (queue_start < queue_end) {
    ...
    if (permeability_array[ii] == 0.0)
      continue;
    if (0) {
      gprintf(goban, "Picked %1m from queue. w=%f start=%d end=%d\n",
	 ...
	 */
	if (0) {
	  gprintf(goban, "  Spreading %s influence from %1m to %1m, d=%d\n",
...
#else
    if (ON_BOARD(goban, ii + delta[0]))
      code1(deltai[0], deltaj[0], ii + delta[0], 0);
    if (ON_BOARD(goban, ii + delta[1]))
      code1(deltai[1], deltaj[1], ii + delta[1], 0);
    if (ON_BOARD(goban, ii + delta[2]))
      code1(deltai[2], deltaj[2], ii + delta[2], 0);
    if (ON_BOARD(goban, ii + delta[3]))
    ...
    if (ON_BOARD(goban, ii + delta[4]))
      code1(deltai[4], deltaj[4], ii + delta[4], 1);
    if (ON_BOARD(goban, ii + delta[5]))
      code1(deltai[5], deltaj[5], ii + delta[5], 1);
    if (ON_BOARD(goban, ii + delta[6]))
      code1(deltai[6], deltaj[6], ii + delta[6], 1);
    if (ON_BOARD(goban, ii + delta[7]))
...
static void
init_influence(const Goban *goban, struct influence_data *q, int color,
	       ...
	       const float strength[BOARDMAX])
{
  ...
  if (cosmic_gnugo) {
    ...
    if (goban->board_size != 19
	|| goban->move_number <= 2
	|| (goban->move_number / 2) % 2)
      cosmic_importance = 0.0;
    else {
      cosmic_importance = 1.0 - ((goban->move_number / 150.0)
				 * (goban->move_number / 150.0));
  ...
  q->captured = goban->black_captured - goban->white_captured;
 ...
 */
static void
enter_intrusion_source(const Goban *goban, int source_pos, int strength_pos,
		       float strength, float attenuation,
		       struct influence_data *q)
{
  if (q->intrusion_counter >= MAX_INTRUSIONS) {
    DEBUG(goban, DEBUG_INFLUENCE, "intrusion list exhausted\n");
    return;
  }
 ...
 */
static void
reset_unblocked_blocks(const Goban *goban, struct influence_data *q)
{
  int pos;
  for (pos = BOARDMIN; pos < BOARDMAX; pos++)
	...
	DEBUG(goban, DEBUG_INFLUENCE, "  black block removed from %1m\n", pos);
	q->white_permeability[pos] = 1.0;
      }
      if (!q->safe[pos] && q->black_strength[pos] > 0.0
	  && q->black_permeability[pos] != 1.0) {
	DEBUG(goban, DEBUG_INFLUENCE, "  white block removed from %1m\n", pos);
	q->black_permeability[pos] = 1.0;
      }
    }
}
 ...
 */
static void
add_marked_intrusions(const Goban *goban, struct influence_data *q, int color)
{
  int i;
  int j = 0;
  int source_pos;
  ...
  for (i = 0; i < q->intrusion_counter; i = j) {
    ...
    if (source_pos == NO_MOVE) {
      ...
      DEBUG(goban, DEBUG_INFLUENCE, "Adding %s intrusion at %1m, value %f\n",
 ...
 */
static void
influence_callback(Goban *goban, int anchor, int color,
		   struct pattern *pattern, int ll, void *data)
{
  ...
  if ((pattern->class & (CLASS_D | CLASS_A | CLASS_B | CLASS_E | CLASS_t))
	...
	/* FIXME: This test is probably not necessary any more. */
	  ...
	  if ((goban->stackp == 0 && worm[ii].attack_codes[0] != 0)
	...
	if (pattern->class & CLASS_D) {
	  gg_assert(goban, pattern->patn[k].att == ATT_O);
	  if ((color == WHITE && q->white_strength[ii] == 0.0)
	      || (color == BLACK && q->black_strength[ii] == 0.0))
	    return; /* Match failed. */
	}
  ...
  if (pattern->autohelper_flag & HAVE_CONSTRAINT) {
    if (!pattern->autohelper(goban, ll, pos, color, 0))
      return;
  }
   ...
   */
  if (pattern->helper) {
    if (!pattern->helper(goban, pattern, ll, pos, color)) {
      DEBUG(goban, DEBUG_INFLUENCE,
  ...
  DEBUG(goban, DEBUG_INFLUENCE, "influence pattern '%s'+%d matched at %1m\n",
  ...
  if ((pattern->class & CLASS_t)
    ...
    pattern->autohelper(goban, ll, pos, color, INFLUENCE_CALLBACK);
  }
  ...
  for (k = 0; k < pattern->patlen; ++k) { /* match each point */
	...
	if (pattern->class & CLASS_D)
	  ...
	  blocking_color = OTHER_COLOR(color);
	DEBUG(goban, DEBUG_INFLUENCE, "  barrier for %s influence at %1m\n",
	...
	if (q->is_territorial_influence)
	  enter_intrusion_source(goban, anchor, ii, strength,
				 TERR_DEFAULT_ATTENUATION, q);
	else
	  add_influence_source(ii, color, strength,
				 DEFAULT_ATTENUATION, q);
	DEBUG(goban, DEBUG_INFLUENCE, "  intrusion at %1m\n", ii);
      }
 ...
 */
static void
followup_influence_callback(Goban *goban, int anchor, int color,
			    struct pattern *pattern, int ll, void *data)
{
  int k;
  int t;
   ...
   */
  if (pattern->autohelper_flag & HAVE_CONSTRAINT) {
    if (!pattern->autohelper(goban, ll, t, color, 0))
      return;
  }
   ...
   */
  if (pattern->helper) {
    if (!pattern->helper(goban, pattern, ll, t, color)) {
      DEBUG(goban, DEBUG_INFLUENCE,
	    "Influence pattern %s+%d rejected by helper at %1m\n",
	    pattern->name, ll, t);
      return;
 ...
 if ((pattern->autohelper_flag & HAVE_ACTION)
     && !pattern->autohelper(goban, ll, t, color,
  ...
  for (k = 0; k < pattern->patlen; ++k)  /* match each point */
    if (pattern->patn[k].att == ATT_not) {
      ...
      /* Low intensity influence source for the color in turn to move. */
      enter_intrusion_source(goban, anchor, ii, pattern->value,
			     TERR_DEFAULT_ATTENUATION, q);
      DEBUG(goban, DEBUG_INFLUENCE, "  followup for %1m: intrusion at %1m\n",
	    anchor, ii);
    }
}
 ...
 */
void
influence_mark_non_territory(const Goban *goban, int pos, int color)
{
  DEBUG(goban, DEBUG_INFLUENCE, "  non-territory for %C at %1m\n", color, pos);
 ...
 */
void
influence_erase_territory(const Goban *goban, struct influence_data *q,
			  int pos, int color)
{
  int k;
  ASSERT1(goban, (color == WHITE && q->territory_value[pos] >= 0.0)
	  || (color == BLACK && q->territory_value[pos] <= 0.0), pos);

  current_influence = q;

  q->territory_value[pos] = 0.0;
  influence_mark_non_territory(goban, pos, color);
  for (k = 0; k < 4; k++) {
    if (ON_BOARD(goban, pos + delta[k])) {
      q->territory_value[pos + delta[k]] = 0.0;
      influence_mark_non_territory(goban, pos + delta[k], color);
    }
  }
}
 ...
 */
static void
find_influence_patterns(Goban *goban, struct influence_data *q, int color)
{
  int ii;

  current_influence = q;
  matchpat(goban, influence_callback, ANCHOR_COLOR, &influencepat_db, q, NULL);
  if (color != EMPTY)
    matchpat(goban, influence_callback, color, &barrierspat_db, q, NULL);

  if (q->is_territorial_influence)
    add_marked_intrusions(goban, q, color);
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	...
	&& q->safe[ii]) {
      ...
      for (k = 0; k < 8; k++) {
	int d = delta[k];
	if (ON_BOARD(goban, ii + d) && !q->safe[ii + d]) {
  ...
  reset_unblocked_blocks(goban, q);
}
 ...
 */
static int
check_double_block(const Goban *goban, int color, int pos,
		   const struct influence_data *q)
{
  int k;
  ...
  if (block_neighbors >= 2) {
    ...
    for (k = 0; k < 4; k++) {
      ...
      if (goban->board[neighbor] == EMPTY && permeability[neighbor] == 0.0) {
	...
	int j;
	for (j = 0; j < 4; j++)
	  if (ON_BOARD(goban, neighbor + delta[j]))
    ...
    ASSERT1(goban, ON_BOARD1(goban, smallest_block), pos);
 ...
 */
static void
remove_double_blocks(const Goban *goban, struct influence_data *q,
		     const char inhibited_sources[BOARDMAX])
{
  ...
  int num_blocks = 0;
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (goban->board[ii] == EMPTY
	&& !(inhibited_sources && inhibited_sources[ii])
	&& strength[ii] > 0.0) {
      double_blocks[num_blocks] = check_double_block(goban, q->color_to_move,
						     ii, q);
    ...
    }
  {
    ...
    for (k = 0; k < num_blocks; k++) {
      DEBUG(goban, DEBUG_INFLUENCE, "Removing block for %s at %1m.\n",
	    q->color_to_move, double_blocks[k]);
      permeability[double_blocks[k]] = 1.0;
      accumulate_influence(goban, q, double_blocks[k], q->color_to_move);
    }
  }
}
 ...
 */
static void
do_compute_influence(Goban *goban, int color, const char safe_stones[BOARDMAX],
		     const char inhibited_sources[BOARDMAX],
		     const float strength[BOARDMAX], struct influence_data *q,
		     int move, const char *trace_message)
{
  int ii;
  init_influence(goban, q, color, safe_stones, strength);

  modify_depth_values(goban->stackp - 1);
  if (q != &escape_influence)
    find_influence_patterns(goban, q, color);
  else
    find_influence_patterns(goban, q, EMPTY);
  modify_depth_values(1 - goban->stackp);

  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (ON_BOARD(goban, ii) && !(inhibited_sources && inhibited_sources[ii])) {
      if (q->white_strength[ii] > 0.0)
	accumulate_influence(goban, q, ii, WHITE);
      if (q->black_strength[ii] > 0.0)
	accumulate_influence(goban, q, ii, BLACK);
    }

  value_territory(goban, q);
  remove_double_blocks(goban, q, inhibited_sources);

  value_territory(goban, q);
  segment_influence(goban, q);

  if ((move == NO_MOVE
       && (printmoyo & PRINTMOYO_INITIAL_INFLUENCE))
      || (debug_influence && move == debug_influence))
    print_influence(goban, q, trace_message);
}
...
void
compute_influence(Goban *goban, int color, const char safe_stones[BOARDMAX],
		  const float strength[BOARDMAX], struct influence_data *q,
		  int move, const char *trace_message)
{
  ...
  do_compute_influence(goban, color, safe_stones, NULL, strength,
 ...
 */
int
whose_territory(const Goban *goban, const struct influence_data *q, int pos)
{
 ...
 */
int
whose_loose_territory(const Goban *goban,
		      const struct influence_data *q, int pos)
{
  ...
  if (ON_BOARD(goban, pos) && IS_STONE(goban->board[pos]) && !q->safe[pos]) {
    if (0)
      TRACE(goban, "dead stone : %1m, territory for %s\n", pos,
		  (owner == WHITE ? "White":
		  (owner == BLACK ? "Black": "nobody????")));
    owner = OTHER_COLOR(goban->board[pos]);
  }
 ...
 */
int
whose_moyo(const Goban *goban, const struct influence_data *q, int pos)
{
  float bi = q->black_influence[pos];
  float wi = q->white_influence[pos];

  int territory_color = whose_territory(goban, q, pos);
 ...
 */
static int
whose_moyo_restricted(const Goban *goban,
		      const struct influence_data *q, int pos)
{
  float bi = q->black_influence[pos];
  float wi = q->white_influence[pos];
 ...
 */
int
whose_area(const Goban *goban, const struct influence_data *q, int pos)
{
  float bi = q->black_influence[pos];
  float wi = q->white_influence[pos];

  int moyo_color = whose_moyo(goban, q, pos);
  if (moyo_color != EMPTY)
    return moyo_color;
...
static void
value_territory(const Goban *goban, struct influence_data *q)
{
  int ii;
  int dist_i, dist_j;
  float central;
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	 ...
	 *  4  5  6  7  7  7  7  7  6  5  4
	 */
	dist_i = gg_min(I(ii), goban->board_size - I(ii) - 1);
	dist_j = gg_min(J(ii), goban->board_size - J(ii) - 1);
	if (dist_i > dist_j)
 ...
 */
static void
segment_region(const Goban *goban, struct influence_data *q,
	       ...
	       int type, int segmentation[BOARDMAX])
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (ON_BOARD(goban, ii)) {
      if ((!marked[ii]) && region_owner(goban, q, ii) != EMPTY) {
	...
	int queue_start = 0;
	int queue_end = 1;
	int color = region_owner(goban, q, ii);
	q->number_of_regions++;
	marked[ii] = 1;
	q->queue[0] = ii;
	while (queue_start < queue_end) {
	  ...
	  int k;
	  queue_start++;
	  if (!q->safe[tt] || goban->board[tt] != color) {
	    size++;
	    if (q->is_territorial_influence)
	  ...
	  for (k = 0; k < 4; k++) {
	    int d = delta[k];
	    if (ON_BOARD(goban, tt + d)
		&& !marked[tt + d]
		&& region_owner(goban, q, tt + d) == color) {
	...
	if (0)
	  gprintf(goban, "Region %d of type %d (color %s) at %1m. Size %d\n",
 ...
 */
static void
segment_influence(const Goban *goban, struct influence_data *q)
{
  int ii;
  ...
  if (use_optimistic_territory) {
    segment_region(goban, q, whose_loose_territory,
		   IS_TERRITORY, q->territory_segmentation);
  }
  else {
    segment_region(goban, q, whose_territory,
		   IS_TERRITORY, q->territory_segmentation);
  }

  segment_region(goban, q, whose_moyo_restricted,
		 IS_MOYO, q->moyo_segmentation);
  segment_region(goban, q, whose_area,
		 IS_AREA, q->area_segmentation);
}


/* Export the moyo segmentation. */
void
influence_get_moyo_segmentation(const Goban *goban,
				const struct influence_data *q,
...
void
influence_get_territory_segmentation(const Goban *goban,
				     const struct influence_data *q,
				     struct moyo_data *moyos)
{
  int ii;
...
void
influence_get_moyo_data(const Goban *goban, const struct influence_data *q,
			...
			float territory_value[BOARDMAX])
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (ON_BOARD(goban, ii)) {
      moyo_color[ii] = whose_moyo_restricted(goban, q, ii);
...
int
influence_considered_lively(const Goban *goban,
			    const struct influence_data *q, int pos)
{
  int color = goban->board[pos];
...
void
resegment_initial_influence(const Goban *goban)
{
  segment_influence(goban, &initial_black_influence);
  segment_influence(goban, &initial_white_influence);
}
 ...
 */
void
compute_followup_influence(Goban *goban, const struct influence_data *base,
			   struct influence_data *q,
			   int move, const char *trace_message)
{
  int ii;
  ...
  ASSERT1(goban, IS_STONE(q->color_to_move), move);
  ...
  matchpat_goal_anchor(goban, followup_influence_callback, color,
		       &barrierspat_db, q, goal, 1);
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	...
	accumulate_influence(goban, q, ii, color);
...
void
compute_escape_influence(Goban *goban, int color,
			 const char safe_stones[BOARDMAX],
			 ...
			 char escape_value[BOARDMAX])
{
  ...
  if (!goal) {
     ...
     */
    for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
      if (cached_board[ii] != goban->board[ii]) {
	cached_board[ii] = goban->board[ii];
	board_was_cached = 0;
      }
    }
  ...
  do_compute_influence(goban, OTHER_COLOR(color), safe_stones, goal, strength,
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
    if (ON_BOARD(goban, ii)) {
      if (whose_moyo(goban, &escape_influence, ii) == color)
	escape_value[ii] = 4;
      else if (whose_area(goban, &escape_influence, ii) == color)
	escape_value[ii] = 2;
      else if (whose_area(goban, &escape_influence, ii) == EMPTY) {
	if (goal) {
	  escape_value[ii] = 0;
	  ...
	  if (!goal[ii]) {
		...
		if (k < 4 && ON_BOARD(goban, ii + 2 * delta[k]))
  ...
  if (0 && (debug & DEBUG_ESCAPE) && verbose > 0)
    print_influence(goban, &escape_influence, "escape influence");
 ...
 */
int
retrieve_delta_territory_cache(const Goban *goban,
			       ...
			       Hash_data safety_hash)
{
   ...
   */
  if (territory_cache_position_number == goban->position_number
    ...
    *followup_value = followup_territory_cache[pos];
    if (0)
      gprintf(goban, "%1m: retrieved territory value from cache: %f, %f\n", pos,
...
void
store_delta_territory_cache(const Goban *goban, int pos, int color,
			    ...
			    Hash_data safety_hash)
{
  ...
  if (territory_cache_position_number != goban->position_number
    ...
    territory_cache_position_number = goban->position_number;
    territory_cache_influence_id = base->id;
    territory_cache_color = color;
    if (0)
      gprintf(goban, "Cleared delta territory cache.\n");
  }
  ...
  if (0)
    gprintf(goban, "%1m: Stored delta territory cache: %f, %f\n", pos, move_value,
	    followup_value);
}
 ...
 */
float
influence_delta_territory(const Goban *goban,
			  const struct influence_data *base,
			  ...
			  int move)
{
  ...
  ASSERT1(goban, IS_STONE(color), move);
...

  {
    ...
    if (move != -1
	&& this_delta != 0.0)
      DEBUG(goban, DEBUG_TERRITORY, "  %1m:   - captured stones %f\n",
	    move, this_delta);
    total_delta += this_delta;
  }
  return total_delta;
 ...
 */
float
influence_score(const Goban *goban, const struct influence_data *q)
{
  float score = 0.0;
  int ii;
  ...
  score += goban->black_captured - goban->white_captured + goban->komi;
 ...
 */
static float
influence_evaluate_position(const Goban *goban, int color,
			    float *power, float *game_status)
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	...
	score += (goban->board[ii] == WHITE ? 1 : -1);
	count += WEIGHT_TERRITORY;
      }
      else if (whose_territory(goban, iq, ii) != EMPTY
	       && whose_territory(goban, oq, ii) != EMPTY) {
	  /* chinese-style scoring, add 1 point max, not 2 */
	score += (iq->territory_value[ii] > 0 ? 1 : -1);
	count += WEIGHT_TERRITORY;
      }
      else if (whose_moyo(goban, oq, ii) != EMPTY) {
	power_balance += oq->white_influence[ii] - oq->black_influence[ii];
	non_territory++;
	count += WEIGHT_MOYO;
      }
      else if (whose_area(goban, oq, ii) != EMPTY) {
	power_balance += oq->white_influence[ii] - oq->black_influence[ii];
	non_territory++;
	count += WEIGHT_AREA;
      }
  ...
  if (game_status) {
    *game_status = ((float) count
		    / (WEIGHT_TERRITORY
		       * goban->board_size * goban->board_size));
  }
 ...
 */
float
game_status(const Goban *goban, int color)
{
  float status;
  influence_evaluate_position(goban, color, NULL, &status);
  return status;
}
 ...
 */
void
get_influence(const Goban *goban, const struct influence_data *q,
	      ...
	      int non_territory[BOARDMAX])
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
    ...
    if (goban->board[ii] == EMPTY) {
      if (whose_territory(goban, q, ii) == WHITE)
	influence_regions[ii] = 3;
      else if (whose_territory(goban, q, ii) == BLACK)
	influence_regions[ii] = -3;
      else if (whose_moyo(goban, q, ii) == WHITE)
	influence_regions[ii] = 2;
      else if (whose_moyo(goban, q, ii) == BLACK)
	influence_regions[ii] = -2;
      else if (whose_area(goban, q, ii) == WHITE)
	influence_regions[ii] = 1;
      else if (whose_area(goban, q, ii) == BLACK)
	influence_regions[ii] = -1;
      else
 ...
 */
void
print_influence(const Goban *goban, const struct influence_data *q,
		const char *info_string)
{
  if (printmoyo & PRINTMOYO_ATTENUATION) {
    /* Print the attenuation values. */
    fprintf(stderr, "white attenuation (%s):\n", info_string);
    print_numeric_influence(goban, q, q->white_attenuation, "%3.2f", 3, 0, 0);
    fprintf(stderr, "black attenuation (%s):\n", info_string);
    print_numeric_influence(goban, q, q->black_attenuation, "%3.2f", 3, 0, 0);
  }

  if (printmoyo & PRINTMOYO_PERMEABILITY) {
    /* Print the white permeability values. */
    fprintf(stderr, "white permeability:\n");
    print_numeric_influence(goban, q, q->white_permeability, "%3.1f", 3, 0, 0);

    /* Print the black permeability values. */
    fprintf(stderr, "black permeability:\n");
    print_numeric_influence(goban, q, q->black_permeability, "%3.1f", 3, 0, 0);
  }
  ...
  if (printmoyo & PRINTMOYO_STRENGTH) {
    ...
    if (q->is_territorial_influence)
      print_numeric_influence(goban, q, q->white_strength, "%5.1f", 5, 0, 0);
    else
      print_numeric_influence(goban, q, q->white_strength, "%3.0f", 3, 0, 1);
    fprintf(stderr, "black strength:\n");
    if (q->is_territorial_influence)
      print_numeric_influence(goban, q, q->black_strength, "%5.1f", 5, 0, 0);
    else
      print_numeric_influence(goban, q, q->black_strength, "%3.0f", 3, 0, 1);
  }

  if (printmoyo & PRINTMOYO_NUMERIC_INFLUENCE) {
    /* Print the white influence values. */
    fprintf(stderr, "white influence (%s):\n", info_string);
    print_numeric_influence(goban, q, q->white_influence, "%5.1f", 5, 1, 0);
    /* Print the black influence values. */
    fprintf(stderr, "black influence (%s):\n", info_string);
    print_numeric_influence(goban, q, q->black_influence, "%5.1f", 5, 1, 0);
  }

  if (printmoyo & PRINTMOYO_PRINT_INFLUENCE) {
    fprintf(stderr, "influence regions (%s):\n", info_string);
    print_influence_areas(goban, q);
  }
  if (printmoyo & PRINTMOYO_VALUE_TERRITORY) {
    fprintf(stderr, "territory (%s)", info_string);
    print_numeric_influence(goban, q, q->territory_value, "%5.2f", 5, 1, 0);
  }
}
 ...
 */
static void
print_numeric_influence(const Goban *goban, const struct influence_data *q,
			const float values[BOARDMAX],
			const char *format, int width,
			int draw_stones, int mark_epsilon)
{
  ...
  fprintf(stderr, "   ");
  for (i = 0, ch = 'A'; i < goban->board_size; i++, ch++) {
    if (ch == 'I')
      ch++;
...
static void
print_influence_areas(const Goban *goban, const struct influence_data *q)
{
  int ii;
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++)
	...
	if (goban->board[ii] == WHITE)
      ...
      else if (whose_territory(goban, q, ii) == WHITE) {
	c = 'o';
	color = GG_COLOR_CYAN;
      }
      else if (whose_territory(goban, q, ii) == BLACK) {
	...
	color = GG_COLOR_RED;
      ...
      draw_color_char(goban->board_size, I(ii), J(ii), c, color);
    }
