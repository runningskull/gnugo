...
find_double_threats(int color)
{
  ...
  for (ii = BOARDMIN; ii < BOARDMAX; ii++) {
    ...
    if (num_a_threatened_groups > 1) {
      if (trymove(goban, ii, color, "find_double_threats-A", ii)) {
	for (k = 0; k < num_a_threatened_groups - 1; ++k)
	  for (l = k + 1; l < num_a_threatened_groups; ++l) {
	    /* Note: If we used attack_either() here instead of trymove(goban)
	    ...
	    int save_verbose)
{
  ...
  if (defense_moves) {
	...
	continue;
      ...
      if (!trymove(goban, pos, other, "atari_atari", NO_MOVE)) {
	defense_moves[pos] = 0;
	if (save_verbose)
	  gprintf(goban, "%1m deleted defense point, illegal\n", pos);
	continue;
      }
	...
	popgo(goban);
	if (save_verbose)
	  gprintf(goban, "%1m deleted defense point, unsafe\n", pos);
	continue;
      }
			 ...
			 const char safe_stones[BOARDMAX])
{
  ...
  if (defense_moves) {
    ...
    for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
      if (!ON_BOARD(goban, pos) || !defense_moves[pos] || pos == move)
	continue;

      if (!trymove(goban, pos, color, "atari_atari", NO_MOVE)) {
	defense_moves[pos] = 0;
	continue;
      }
      increase_depth_values();
	       ...
	       int save_verbose, int minsize, char goal[BOARDMAX])
{
  ...
  if (debug & DEBUG_ATARI_ATARI) {
    ...
    dump_stack(goban);
    gprintf(goban, "%oforbidden moves: ");
    for (pos = BOARDMIN; pos < BOARDMAX; pos++)
      if (ON_BOARD(goban, pos) && forbidden[pos])
  ...
  for (k = 0; attacks[k].move != NO_MOVE; k++) {
    ...
    if (!trymove(goban, apos, color, "do_atari_atari-A", str))
      continue;
	...
	int libs[2];
	int num_libs = findlib(goban, apos, 2, libs);
	...
	all_potential_defenses[bpos] = 1;
      ...
      if (trymove(goban, bpos, other, "do_atari_atari-B", str)) {
	int new_aa_val;
			      ...
			      char goal[BOARDMAX])
{
  ...
  if (debug & DEBUG_ATARI_ATARI) {
    ...
    for (k = 0; k < AA_MAX_MOVES && attacks[k].move != NO_MOVE; k++) {
      gprintf(goban, "%o %1m(", attacks[k].move);
      for (r = 0; r < AA_MAX_TARGETS_PER_MOVE; r++) {
	if (attacks[k].target[r] == NO_MOVE)
	  break;
	gprintf(goban, "%o%s%1m", r == 0 ? "" : ",", attacks[k].target[r]);
      }
      gprintf(goban, "%o)");
    }
    gprintf(goban, "%o\n");
			    ...
			    struct pattern *pattern, int ll, void *data)
{
  ...
  for (k = 0; k < pattern->patlen; ++k) { /* match each point */
    if (pattern->patn[k].att == ATT_X) {
      ...
      int str = find_origin(goban, AFFINE_TRANSFORM(pattern->patn[k].offset,
					     ll, anchor));
	...
	continue;
       ...
       */
      if (!(pattern->class & CLASS_s) && is_self_atari(goban, move, color)) {
			       ...
			       int moves[AA_MAX_MOVES])
{
  ...
  for (r = 0; r < AA_MAX_TARGETS_PER_MOVE && targets[r] != NO_MOVE; r++) {
	...
	mx[attack_point] = 1;
       ...
       */
      liberties = findlib(goban, adjs[k], 3, libs);
      if (liberties <= 3) {
    ...
    if (debug & DEBUG_ATARI_ATARI) {
      gprintf(goban, "Defense moves for %1m:", str);
      for (k = 0; k < num_moves; k++)
	gprintf(goban, "%o %1m", moves[k]);
      gprintf(goban, "%o\n");
    }
  }
...
compute_aa_values(int color)
{
  ...
  for (pos = BOARDMIN; pos < BOARDMAX; pos++) {
    ...
    aa_values[pos] = value;
    if (1)
      DEBUG(goban, DEBUG_ATARI_ATARI, "aa_value for %1m = %d\n", pos, value);
  }
}
...
get_aa_value(int str)
{
  int stones[MAX_BOARD * MAX_BOARD];
  int k;
  int num_stones = findstones(goban, str, MAX_BOARD * MAX_BOARD, stones);
  int value = 0;
	       ...
	       int color)
{
  ...
  for (r = 0; r < queue_end; r++) {
    ...
    gg_assert(goban, queue_end < MAX_BOARD * MAX_BOARD);
