...
int main(int argc, array(string) argv)
{
    ...
    GTK.setup_gtk(argv);
    RegressionViewer viewer = RegressionViewer(engine, argv[1]);
...
//    GTK.RadioButton initial_b_influence_dragons_unknown_button;
	...
	board_window->set_title(testcase);
...
//			      ->add(initial_b_influence_dragons_unknown_button)
	...
	data_window->set_title(testcase);
	data_window->show_all();

	all_windows = ({board_window, notebook_window, data_window});
	...
	if (dragon_status_button->get_active())
	{
	...
	else if (dragon_safety_button->get_active())
	{
	    foreach (dragons; string dragon; array(string) stones)
	    {
	...
	else if (worm_status_button->get_active())
	{
    ...
    {
	if ((top_moves_button->get_active()
	     || all_moves_button->get_active())
	    && (has_prefix(testcase_command, "reg_genmove")
		|| has_prefix(testcase_command, "restricted_genmove")))
	{
	    string color = "green";
	    string answers = expected_result;
	    if (answers[0..0] == "!") {
		answers = answers[1..];
		color = "red";
	    }
	...
	else if (all_moves_button->get_active())
	{
	...
	else if (delta_territory_button->get_active()
		 && delta_territory_move != "PASS")
	{
	    goban->add_symbol(delta_territory_move, "stone", "gray");
	    delta_territory_button_text->set_text("delta territory for "
						  + delta_territory_move);
	    clist->clear();

	    int k;
	    for (k = sizeof(traces) - 1; k >= 0; k--)
	    {
		if (sscanf(traces[k], "  " + delta_territory_move
	...
	string color;
	if (white_eyes_button->get_active())
	    color = "white";
	else
	    color = "black";
	...
	string command;
	string what_data;
	if (initial_w_influence_dragons_known_button->get_active())
	    command = "initial_influence white";
	else if (initial_b_influence_dragons_known_button->get_active())
	    command = "initial_influence black";
	else if (after_move_influence_button->get_active())
	{
	    if (move_influence_move == "PASS")
		return;
	    command = sprintf("move_influence %s %s", current_move_color,
			      move_influence_move);
	    goban->add_symbol(move_influence_move, "stone", "gray");
	}
	else if (followup_influence_button->get_active())
	{
	    if (move_influence_move == "PASS")
		return;
	    command = sprintf("followup_influence %s %s", current_move_color,
			      move_influence_move);
	    goban->add_symbol(move_influence_move, "stone", "gray");
	}

	if (influence_regions_button->get_active())
	    what_data = "influence_regions";
	if (territory_value_button->get_active())
	    what_data = "territory_value";
	else if (white_influence_button->get_active())
	    what_data = "white_influence";
	else if (black_influence_button->get_active())
	    what_data = "black_influence";
	else if (white_influence_button->get_active())
	    what_data = "white_influence";
	else if (black_influence_button->get_active())
	    what_data = "black_influence";
	else if (white_strength_button->get_active())
	    what_data = "white_strength";
	else if (black_strength_button->get_active())
	    what_data = "black_strength";
	else if (white_permeability_button->get_active())
	    what_data = "white_permeability";
	else if (black_permeability_button->get_active())
	    what_data = "black_permeability";
	else if (white_attenuation_button->get_active())
	    what_data = "white_attenuation";
	else if (black_attenuation_button->get_active())
	    what_data = "black_attenuation";
	else if (non_territory_button->get_active())
	...
	if ((connection_reading_button->get_active()
	    ...
	    goban->add_symbol(first_semeai_or_connection_vertex, "big_dot",
			      "green");
    }
    
    static void show_worm_data(string vertex)
    {
    ...
    static void show_dragon_data(string vertex, int part)
    {
    ...
    static void show_move_reasons(string vertex)
    {
    ...
    static void show_eye_data(string vertex)
    {
	clist->clear();
...
//	werror("Button: %O\n", (mapping) event);
	...
	else if (notebook->get_current_page() == 4)
	{
	    ...
	    goban->add_symbol(vertex, "big_dot", "green");
	    redraw_board();
	    ...
	    {
		string prefix = "";
		string reset_counter = "reset_reading_node_counter";
		string get_counter = "get_reading_node_counter";
		...
		string result = send_command(prefix + "attack " + vertex);
		clist->append(({prefix + "attack " + vertex + "  ", result,
				"(" + send_command(get_counter)
				+ " nodes)"}));
		
		if (result[0..0] != "0")
		{
		    send_command(reset_counter);
		    string result = send_command(prefix + "defend " + vertex);
		    clist->append(({prefix + "defend " + vertex + "  ", result,
				    "(" + send_command(get_counter)
				    + " nodes)"}));
		}

		if (sgf_traces_button->get_active())
		    send_command("finish_sgftrace "
				 + sgf_filename_entry->get_text());
	    }
		...
		else if (first_semeai_or_connection_vertex != vertex)
		{
			...
			c = sprintf("connect %s %s\n",
				    first_semeai_or_connection_vertex,
				    vertex);
			result = send_command(c);
			clist->append(({c + "  ", result,
					"(" + send_command("get_connection_node_counter")
					+ " nodes)"}));
			if (result[0..0] != "0")
			{
			    send_command("reset_connection_node_counter");
			...
			c = sprintf("analyze_semeai %s %s",
				    first_semeai_or_connection_vertex,
				    vertex);
			result = send_command(c);
			clist->append(({c + "  ", result,
					"(" + send_command("get_owl_node_counter")
					+ " nodes)"}));

			send_command("reset_owl_node_counter");
			c = sprintf("analyze_semeai %s %s", vertex,
				    first_semeai_or_connection_vertex);
			result = send_command(c);
			clist->append(({c + "  ", result,
					"(" + send_command("get_owl_node_counter")
	    ...
	    clist->columns_autosize();
	    data_window->set_title("Reading for " + vertex);
	}
    }
...
//	werror("Key: %O\n", (mapping) event);
    ...
    static int load_testcase(string testcase)
    {
	string filename;
	int number;
    ...
    static void debug_callback(mixed ... args)
    {
